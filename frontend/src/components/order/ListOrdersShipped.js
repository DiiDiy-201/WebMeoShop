...
import { myOrdersShipped, clearErrors } from "../../actions/orderActions";

const ListOrdersShipped = () => {
  const alert = useAlert();
  const dispatch = useDispatch();

  const { loading, error, orders } = useSelector(
    (state) => state.myOrdersShipped
  );

  useEffect(() => {
    dispatch(myOrdersShipped());
...
export default ListOrdersShipped;
