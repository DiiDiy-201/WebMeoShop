...
import { myOrdersShipping, clearErrors } from "../../actions/orderActions";

const ListOrdersShipping = () => {
  const alert = useAlert();
  const dispatch = useDispatch();

  const { loading, error, orders } = useSelector(
    (state) => state.myOrdersShipping
  );

  useEffect(() => {
    dispatch(myOrdersShipping());
...
export default ListOrdersShipping;
