...
import { myOrdersCancel, clearErrors } from "../../actions/orderActions";

const ListOrdersCancel = () => {
  const alert = useAlert();
  const dispatch = useDispatch();

  const { loading, error, orders } = useSelector(
    (state) => state.myOrdersCancel
  );

  useEffect(() => {
    dispatch(myOrdersCancel());
...
export default ListOrdersCancel;
