...
import { myOrdersProcessed, clearErrors } from "../../actions/orderActions";

const ListOrdersProcessed = () => {
  const alert = useAlert();
  const dispatch = useDispatch();

  const { loading, error, orders } = useSelector(
    (state) => state.myOrdersProcessed
  );

  useEffect(() => {
    dispatch(myOrdersProcessed());
...
export default ListOrdersProcessed;
