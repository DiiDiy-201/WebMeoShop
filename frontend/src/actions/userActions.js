...
// Get all khachs
export const allKhachs = () => async (dispatch) => {
  try {
    dispatch({ type: ALL_KHACHS_REQUEST });

    const { data } = await axios.get("/api/v1/admin/khachs");

    dispatch({
      type: ALL_KHACHS_SUCCESS,
      payload: data.khachs,
    });
  } catch (error) {
    dispatch({
      type: ALL_KHACHS_FAIL,
      payload: error.response.data.message,
    });
  }
};
